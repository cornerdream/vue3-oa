...
export function sworkflowtask(search: string,ordering: string,page: string,size: string) {
    return $axios({
      url: '/api/user/workflowtask?search='+search+'&ordering='+ordering+'&page='+page+'&size='+size,
      method: 'get'
    })
}
//获取某个审批任务单据详情
...
export function approving(search: string,ordering: string,page: string,size: string) {
  return $axios({
    url: '/api/user/workflowtask/approving?search='+search+'&ordering=_'+ordering+'&page='+page+'&size='+size,
    method: 'get'
  })
}
//获取当前用户的已拒绝任务
export function denyed(search: string,ordering: string,page: string,size: string) {
  return $axios({
    url: '/api/user/workflowtask/denyed?search='+search+'&ordering=_'+ordering+'&page='+page+'&size='+size,
    method: 'get'
  })
}
