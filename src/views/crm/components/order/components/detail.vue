<template>
  <div class="detail">
    ...
    <div class="detail-box">
      ...
      <div class="eltable">
        <el-table :data="form.line" border style="width: 100%; margin-bottom: 20px">
          ...
          <el-table-column prop="estimated_arrival_date" label="预计到货日期" width="180">
            <template #default="scope">
              <span>
...
import { buyerid, roolback, sent, draft, roolcommit, actionval } from '../../../../../api/buyer'
...
export default {
  ...
  mounted() {
    for (let i in this.crumnName) {
      this.crumnName[i].path = sessionStorage.getItem('path')
    }
  },
  methods: {
    async buyerdetail() {
    ...
    handleEdit(item) {
      ...
      vm.form = {
        ...
        department: this.form.department ? this.form.department.id : '',
        project: this.form.project ? this.form.project.id : '',
        supplier: this.form.supplier ? this.form.supplier.id : ''
      }
      this.selectOption = this.form.department ? this.form.department.name : ''
      this.$refs.form.is_deleted = true
    },
    getMsgFormSon(data) {
      this.form = data
      this.form.states = data.states.slice(0, 5).reverse()
