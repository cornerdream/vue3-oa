  ...
  <div class="cart">
    ...
    >
      ...
      <el-table-column prop="count" label="商品数量" width="200">
        <template v-slot="scope">
          <el-input-number size="mini" :min="1" v-model="scope.row.count" @change="changeGateway(scope.row,scope.column,scope.row.count)" :row="scope.row" ref="inputNumber" ></el-input-number>
        </template>
      </el-table-column>
        ...
        <template v-slot="scope">
          ...
          <el-select v-model="value1" placeholder="请选择" class="input-box" >
            <el-option
              ...
              :key="item.id"
              :label="item.name"
              :value="item.id" @click.native="changeGateway(scope.row,scope.column,item)">
            </el-option>
          </el-select>
          
          
          <span @click="handleCellClick($event,scope.row,scope.column)">{{scope.row.buyer.name}}</span>
        </template>
      ...
      <el-table-column prop="notes" label="备注" width="120" >
        <template v-slot="scope">
          <el-input
            ...
            :rows="2"
            placeholder="请输入内容"
            v-model="value2" class="input-box"  @blur.native="changeGateway(scope.row,scope.column,value2)" >
          </el-input>
          <span @click="handleCellClick($event,scope.row,scope.column)">{{scope.row.notes}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="project" label="项目" width="120">
        <template v-slot="scope">
          ...
          <span @click="handleCellClick($event,scope.row,scope.column)">{{scope.row.project.name}}</span>
        </template>
      </el-table-column>
    ...
    <div class="cart-footer">
      <div class="grid-content bg-purple">商品总价格：<span class="totalPrice">{{totalPrice}}</span></div>
    ...
    <el-dialog title="订单创建错误" v-model="dialogErrorVisible" >
      <el-result icon="error" title="错误提示" :subTitle="error">
        <template #extra>
          <el-button type="primary" size="medium"  @click="dialogErrorVisible = false">返回</el-button>
        </template>
      </el-result>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    handleCellClick(event,row,column){       
      ...
      if(column.property=='buyer'||column.property=='project'||column.property=='notes'){
